#!/bin/bash -e
#SBATCH -J bwa_mappingB2
#SBATCH -A ga03186
#SBATCH --time=00:18:00 
#SBATCH --mem=18G 
#SBATCH --cpus-per-task=15
#SBATCH --array=1-11%4 # Tailor to samp number - limited to 12 simultaneously.
#SBATCH --out=%x.%j.out
#SBATCH --err=%x.%j.err
...
fq_list2=/nesi/project/ga03186/scripts/GBS_mapping_bwa_B2/samplist.txt # those samples that failed the first time around
# The Sbatch params have been increased from 10 min, 15 G mem, 12 CPU
...
# Mapping
cd $OUTDIR2
QUERY1=`cat ${fq_list2} | awk -v line=$SLURM_ARRAY_TASK_ID '{if(NR == line) print $0}'`
echo "Task $SLURM_ARRAY_TASK_ID"

echo "Processing $QUERY1"

echo "bwa mem -t 8 ${REF}.fasta ${INDIR2}${QUERY1}.fq.gz"
bwa mem -t 8 ${REF} ${INDIR2}${QUERY1}.1_val_1.fq.gz | samtools view -bSh - | samtools sort - -o ${OUTDIR2}${QUERY1}.sorted.bam
samtools index ${OUTDIR2}${QUERY1}.sorted.bam

# Now let's grab some mapping stats:
echo "Collecting stats"
map=$(samtools view -F4 -c ${OUTDIR2}$QUERY1.sorted.bam)
unmap=$(samtools view -f4 -c ${OUTDIR2}${QUERY1}.sorted.bam)
total=$(($map + $unmap))
perc_mapped=`echo "scale=4;($map/$total)*100" | bc`

echo "$QUERY1.bam" >> ${OUTDIR2}B2_bwa_mapping_stats.txt
echo "mapped $map" >> ${OUTDIR2}B2_bwa_mapping_stats.txt
echo "% mapped $perc_mapped" >> ${OUTDIR2}B2_bwa_mapping_stats.txt
echo "unmapped $unmap" >> ${OUTDIR2}B2_bwa_mapping_stats.txt
